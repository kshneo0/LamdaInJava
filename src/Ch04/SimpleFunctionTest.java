...
public class SimpleFunctionTest {
	...
	public static void main(String[] args) {
		...
		// 나이를 모은다.
		List<Integer> customerAge = new ArrayList<>();

		Function<Customer, Integer> getAge = (Customer c) -> c.getAge();

		for (Customer customer : customers) {
			customerAge.add(getAge.apply(customer));
		}
		
		ToDoubleFunction<List<Integer>> calcAvg = (numList) -> calcAvg(customerAge);
		double avg = calcAvg.applyAsDouble(customerAge);
		
		System.out.println("Input list is : " + customerAge);
		System.out.println("Average number in the list is : " + avg);		
